...
// TODO this is based on the data type requested from LimeSDR, may need to be variable (integer vs. float)
const int numsamples = 10000;   // complex samples per buffer
float buffer[numsamples * 2];   // must hold I+Q values of each sample
...
template <class IN_PORT_TYPE> bool LimeSDR_FEI_i::transmitHelper(IN_PORT_TYPE *dataIn) {
    ...
    static lms_stream_meta_t meta_rx;
    ...
    for (size_t tuner_id = 0; tuner_id < frontend_tuner_status.size(); tuner_id++) {
            ...
            int ret = LMS_SendStream(&(streamId[1]), &packet->dataBuffer.front(), packet->dataBuffer.size() / 2, &meta_rx, timeout_ms);
...
// helper function for getting current channel status, things that change like center frequency, bandwidth, etc. and setting the Redhawk Device fts structure (this gets returned when an allocation is attempted)
