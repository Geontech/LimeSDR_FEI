...
int main(int argc, char** argv)
{
    ...
    if ((numRX = LMS_GetNumChannels(device, LMS_CH_RX)) < 0)
        error();
    cout << "Number of RX channels: " << numRX << endl;
    if ((numTX = LMS_GetNumChannels(device, LMS_CH_TX)) < 0)
        error();
    cout << "Number of TX channels: " << numTX << endl;
    ...
    for (int i = 0; i < numRX; i++) {
		if (LMS_SetLOFrequency(device, LMS_CH_RX, i, 1e9) != 0)
		    error();
    }
    ...
    for (int i = 0; i < numRX; i++) {
		if (LMS_SetLOFrequency(device, LMS_CH_TX, i, 1.2e9) != 0)
		    error();
    }
    ...
    //Set RX gain
    for (int i = 0; i < numRX; i++) {
		if (LMS_SetNormalizedGain(device, LMS_CH_RX, i, 0.7) != 0)
		    error();
    }

    //Set TX gain
    for (int i = 0; i < numRX; i++) {
		if (LMS_SetNormalizedGain(device, LMS_CH_TX, i, 0.4) != 0)
		    error();
    }
    ...
    for (int i = 0; i < numRX; i++) {
    	if (i == 0) {
			if (LMS_SetTestSignal(device, LMS_CH_RX, i, LMS_TESTSIG_NCODIV4, 0, 0) != 0)
				error();
    	}
    	else {
			if (LMS_SetTestSignal(device, LMS_CH_RX, i, LMS_TESTSIG_NCODIV8F, 0, 0) != 0)
				error();
    	}
    }
    ...
    const int chCount = numRX; //number of RX/TX streams
